...
func TestSignerRSA(t *testing.T) {
	...
	withStores(t, func(store Store) {
		withIdentity(t, store, leafRSA, func(ident Identity) {
...
func TestSignerECDSA(t *testing.T) {
	...
	withStores(t, func(store Store) {
		withIdentity(t, store, leafEC, func(ident Identity) {
			signer, err := ident.Signer()
			if err != nil {
				t.Fatal(err)
			}
...
func CertificateHelper(t *testing.T, leaf *fakeca.Identity) {
	withStores(t, func(store Store) {
		withIdentity(t, store, root, func(caIdent Identity) {
			withIdentity(t, store, intermediate, func(interIdent Identity) {
				withIdentity(t, store, leaf, func(leafIdent Identity) {
